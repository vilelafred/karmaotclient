...
function UIItem:onMouseRelease(mousePosition, mouseButton)
  ...
  modules.game_inventory.refreshSlots()
...
function UIItem:onItemChange()
    ...
    if self:getItem() and self:getItem():getTooltip():len() > 0 then
     ...
     if self:getItem() then
		if not modules.game_itemrarity then return end
			local img = modules.game_itemrarity.getImageRarity(self:getItem())
			local rarityFrame = self.colorFrame
			rarityFrame = rarityFrame or g_ui.createWidget("ItemColorFrame", self)
